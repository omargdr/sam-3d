...
def main(args):
    ...
    detector_name = (args.detector_name or "").strip()
    detector_disabled = args.disable_detector or detector_name.lower() in {
        "",
        "none",
        "null",
    }
    if not detector_disabled:
    ...
    for image_path in tqdm(images_list):
        ...
        if args.render_mesh:
            ...
            rend_img = visualize_joints_together(img, outputs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="vitdet",
        type=str,
        help="Human detection model for demo (Default `vitdet`). Set to empty string to disable detector.",
    )
    parser.add_argument(
        "--disable_detector",
        action="store_true",
        default=False,
        help="Disable human detector and run on full image bbox.",
    )
    parser.add_argument(
        "--segmentor_name",
        default="sam2",
    ...
    parser.add_argument(
        ...
        default=1024,
        type=int,
        help="Detector input size. Lower values are faster and usually less accurate.",
    )
    parser.add_argument(
        "--use_mask",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Render Open3D mesh in output visualization (slower).",
    )
    parser.add_argument(
        "--inference_type",
        default="full",
        choices=["full", "body", "hand"],
        help="Inference mode: `body` is typically faster than `full`.",
    )
